...
class TestTask(object):
    ...
    def test_eq(self):
        x = Task(self.signal, dict(some_kwarg="foo"), logger=logging.getLogger("TaskX"))
        y = Task(self.signal, dict(some_kwarg="foo"), logger=logging.getLogger("TaskY"))

        assert x == y

    def test_not_eq(self):
        x = Task(self.signal, dict(some_kwarg="foo", logger=logging.getLogger("TaskX")))
        y = Task(self.signal, dict(some_kwarg="bar", logger=logging.getLogger("TaskY")))

        assert x != y

    def test_unicode(self):
        t = Task(self.signal, dict(some_kwarg="foo"), logger=logging.getLogger("TaskT"))

        assert str(t) == "Mock: {'some_kwarg': 'foo'}"

    def test_get_or_create_gets(self):
        x = Task.get_or_create(
            self.signal, dict(some_kwarg="foo"), logger=logging.getLogger("TaskX")
        )
        y = Task.get_or_create(
            self.signal, dict(some_kwarg="foo"), logger=logging.getLogger("TaskY")
        )
    ...
    def test_get_or_create_creates(self):
        ...
        y = Task.get_or_create(
            self.signal, dict(some_kwarg="bar"), logger=logging.getLogger("TaskY")
        )
    ...
    def test_do_exception(self):
        task_mock = self.get_task_mock("_clean", "_exception", "_completed", "_emit")
    ...
    def test_semaphore(self, inspect):
        slot = mock.Mock()
        slot.side_effect = lambda **k: time.sleep(0.3)

        signal = Signal("tost")
        signal.connect(slot)

        x = Task.get_or_create(
            signal, dict(some_kwarg="foo"), logger=logging.getLogger("TaskX")
        )
        y = Task.get_or_create(
            signal, dict(some_kwarg="foo"), logger=logging.getLogger("TaskY")
        )
